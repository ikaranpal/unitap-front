...
const Pagination = ({ handleNextPage, func }: PagInationProp) => {
	const { checkContractInfo, isErc20Approved, isNftApproved, data } = usePrizeOfferFormContext();
	const approve = data.isNft ? isNftApproved : isErc20Approved;
	return (
		<section className="flex flex-col lg:flex-row w-full max-w-[452px] items-center ">
			<div className="flex flex-col flex-col-reverse sm:flex-row w-full gap-5">
				{func === 'submit' ? (
					...
					<ProviderDashboardButtonNext disabled={checkContractInfo || !approve} onClick={handleNextPage}>
						NEXT
					</ProviderDashboardButtonNext>
				)}
			</div>
